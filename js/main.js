...
function calculateGif(e){
    ...
    if( v >= MIN_EXCELENT_AVERAGE && v <= MAX_GRADE){
        $("#messageGif").html("<img src='img/fuegos.gif' class='img-thumbnail congratulations' />").fadeIn("slow");
...
function message(calc){
    ...
    if( val >= MIN_EXCELENT_AVERAGE){
        $("#message").html("Fantastico!!! Qu&eacute; buen promedio! Felicitaciones!");
    }
    if( val > MAX_GRADE){
        $("#message").html("Algo salio mal al hacer el calculo");
    }
    return val;
}
